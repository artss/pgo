...
function Model (db, table, data) {
    ...
    for (var key in this.fkeys) {
        ...
        keys(this.fkeys[key].model.fields).map();
    }
}
 ...
 *  Delete row
 */
Model.prototype.delete = function(row, callback, errback){
    var self = this;

    if (row instanceof Row) {
        try {
            row.delete(callback, errback);
        } catch (e) {
    ...
    var query = 'delete from '+this.table+' as '+this.table+'_tbl where '+
                build_where(this, row)+' returning '+this.table+'_tbl.*;';
